...
#define     Reserved34		    0x3F  // reserved for production tests
	...
	byte AddicoreRFID_ToCard(byte command, byte *sendData, byte sendLen, byte *backData, uint32_t *backLen);	// RC522 and ISO14443 card communication
	...
	uint _RxBits;	                        //Returns the number of received data bits
	void delay();
	uint8_t reachRFID();
